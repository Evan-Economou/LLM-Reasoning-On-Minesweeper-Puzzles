...
from .dataset import SessionMove, SessionRecord, _now_iso, _session_id, append_session_record, board_from_record, position_to_coord, read_puzzle_dataset
        ...
        for position in sorted(forced_mines, key=lambda item: (item.row, item.col)):
            ...
            moves.append(
                SessionMove(
                    turn=len(moves) + 1,
                    action="FLAG",
                    coordinate=position_to_coord(position),
